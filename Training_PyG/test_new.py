...
X = pd.read_csv(f'../JsonFilesToMatrix/output/top_{num_top}_APIs/app_api_{num_top}_APIs_test.csv', header=0, index_col='Unnamed: 0').sample(1)
y_test = X.pop('Label')
...
model.eval()
pred = model(predict_dataset).argmax(dim=1)
